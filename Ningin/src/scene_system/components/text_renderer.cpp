...
Text::Text(std::string& fontName, std::string& shaderName, Color& textColor, std::string& text, uint8_t fontSize)
	: isStarted(false), shader(resourceManager.getShader(shaderName)), font(resourceManager.getFont(fontName)),
...
void Text::setUserUniforms(std::function<void()> initFunc, std::function<void()> initDrawingFunc,
