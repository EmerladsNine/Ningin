...
void SceneLoader::LoadSceneFromFile(std::string path)
{
	...
	std::vector<uint8_t> file = FileReader::ReadFileBytes(path);
...
void SceneLoader::AddComponent(uint8_t id, EntityId entityId, World& world, const std::vector<std::string>& strings,
	const std::vector<uint32_t>& propertiesPointers, uint8_t* filePropertiesPointer)
{
	...
	case 1: {
		...
		for (uint32_t propertyPointer : propertiesPointers)
		{
			...
			{
				tintingColor = Color(ReadU8(&currentPointer), ReadU8(&currentPointer), ReadU8(&currentPointer),
					ReadU8(&currentPointer));
			}
	...
	case 2: {
		...
		for (uint32_t propertyPointer : propertiesPointers)
		{
			...
			{
				textColor = Color(ReadU8(&currentPointer), ReadU8(&currentPointer), ReadU8(&currentPointer),
					ReadU8(&currentPointer));
			}
			else if (name == "Text")
			{
